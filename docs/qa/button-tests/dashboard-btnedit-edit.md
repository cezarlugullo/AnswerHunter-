...
- Contexto: `dashboard`
- Elemento: `#btnEdit` em `src/dashboard/discipline-detail.html:137`
- Evento: `click`
- Handler: `async (` (src/dashboard/discipline-detail.html:282)
...
Arquivo DOM: `C:\Users\cezar\OneDrive\Área de Trabalho\AnswerHunter\src\dashboard\discipline-detail.html:137`
Arquivo handler: `C:\Users\cezar\OneDrive\Área de Trabalho\AnswerHunter\src\dashboard\discipline-detail.html:282`

Trecho mínimo do binding:
```js
280:       document.getElementById('btnNewModule')?.addEventListener('click', createModule);
281: 
282:       document.getElementById('btnEdit')?.addEventListener('click', async () => {
283:         const newName = prompt('Novo nome:', discipline.name);
284:         if (newName && newName.trim()) {
```
